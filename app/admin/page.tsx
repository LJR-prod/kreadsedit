...
// ── Admin API helpers (pass token in header) ───────────────
function adminToken(): string {
  if (typeof window === 'undefined') return ''
  const t = localStorage.getItem('kreb_admin')
  return t || ''
}
...
export default function AdminPage() {
  ...
  async function addEntry() {
    if (!session || !newEditor || !newUrl) return
    setSaving(true)
    const res = await apiFetch('/api/admin/entry', {
      method: 'POST',
      body: JSON.stringify({ session_id: session.id, editor_name: newEditor, youtube_url: newUrl }),
    })
    if (res.ok) { setNewEditor(''); setNewUrl(''); await load() }
    setSaving(false)
  }
  ...
  // ── Login screen ───────────────────────────────────────────
  if (!authed) return (
    <div style={{ padding:'60px 20px', maxWidth:340, margin:'0 auto', textAlign:'center' }}>
      <div className="font-display" style={{ fontSize:36, fontWeight:900, textTransform:'uppercase', marginBottom:24 }}>Admin</div>
    ...
    <div>
      ...
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'16px 20px', borderBottom:'var(--border)' }}>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:8 }}>
          <div className="font-display" style={{ fontSize:11, fontWeight:700, letterSpacing:'0.08em', textTransform:'uppercase', background:'var(--turq)', color:'var(--ink)', padding:'5px 12px', borderRadius:4 }}>
            Admin
          </div>
            ...
            <div style={{ padding:'16px 20px' }}>
              {!session ? (
                <>
                  ...
                  <input className="k-field" placeholder="Label (ex: Avril 2025)" value={newLabel}
                    onChange={e => setNewLabel(e.target.value)} style={{ marginBottom:10 }} />
                  <input className="k-field" placeholder="Mois (ex: 2025-04)" value={newMonth}
                    onChange={e => setNewMonth(e.target.value)} style={{ marginBottom:12 }} />
                ...
                <>
                  ...
                  <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:8, marginBottom:16 }}>
                    {[['Montages', entries.length], ['Votants', votes.length], ['Statut', session.status]].map(([l, v]) => (
                      <div key={l as string} style={{ background:'white', border:'var(--border)', borderRadius:'var(--radius)', padding:12 }}>
                        ...
                        <div className="font-display" style={{ fontSize:20, fontWeight:900, textTransform:'uppercase' }}>{v}</div>
                      </div>
                    ))}
                  </div>
                  ...
                  <div style={{ display:'flex', gap:8, marginBottom:20, flexWrap:'wrap' }}>
                    {session.status === 'open' && (
                      <button className="btn-sm danger" onClick={closeSession}>Fermer session</button>
                    )}
                    {(session.status === 'open' || session.status === 'closed') && (
                      <button className="btn-sm turq" disabled={saving} onClick={revealSession}>
                        {saving ? '...' : 'Cérémonie →'}
                      </button>
                    )}
                    {session.status === 'revealed' && (
                      <button className="btn-sm filled" onClick={() => router.push('/ceremony')}>
                        Voir la cérémonie →
                      </button>
                    )}
                  </div>
                  ...
                  {/* Add entry form */}
                  {session.status === 'open' && (
                    <div style={{ marginBottom:20, padding:16, background:'white', border:'var(--border)', borderRadius:'var(--radius)' }}>
                      <div className="font-display" style={{ fontSize:11, fontWeight:900, letterSpacing:'0.08em', textTransform:'uppercase', marginBottom:10 }}>
                        Ajouter une soumission
                      </div>
                      <input className="k-field" placeholder="Nom du monteur (ex: Loïs V.)" value={newEditor}
                        onChange={e => setNewEditor(e.target.value)} style={{ marginBottom:8 }} />
                      <input className="k-field" placeholder="URL YouTube" value={newUrl}
                        onChange={e => setNewUrl(e.target.value)} style={{ marginBottom:10 }} />
                      <button className="btn-sm filled" disabled={!newEditor || !newUrl || saving} onClick={addEntry}>
                        {saving ? '...' : 'Ajouter'}
                      </button>
                    </div>
                  )}
                  ...
                  {entries.length === 0
                    ...
                    : entries.map(entry => (
                        <EntryRow key={entry.id} entry={entry} canDelete={session.status === 'open'}
                    ...
                    return (
                      <div key={v.id} style={{ display:'flex', alignItems:'center', gap:12, padding:'10px 0', borderBottom:'1px solid #0d0d0d12' }}>
                        ...
                        <span style={{ fontSize:12, color:'#888' }}>
                          🥇#{g?.display_number ?? '?'} · 🥈#{s?.display_number ?? '?'} · 🥉#{b?.display_number ?? '?'}
                        </span>
                      </div>
                    )
                  })
                    ...
                    <div key={sc.id} style={{ display:'flex', alignItems:'center', gap:12, padding:'10px 0', borderBottom:'1px solid #0d0d0d12' }}>
                      ...
                      <div style={{ flex:1 }}>
                        ...
                        <div style={{ fontSize:11, color:'#888' }}>
                          {sc.wins} victoire{sc.wins > 1 ? 's' : ''}
                          {sc.badges?.length > 0 && <> · {sc.badges.join(', ')}</>}
                        </div>
                      </div>
                      <span className="font-display" style={{ fontSize:14, fontWeight:900, color: i === 0 ? 'var(--gold)' : '#888' }}>
                        {sc.total_points} pts
                      </span>
                    </div>
                  ))
...
function SectionHead({ title }: { title: string }) {
  return (
    <div className="font-display" style={{ fontSize:13, fontWeight:900, letterSpacing:'0.06em', textTransform:'uppercase', marginBottom:14 }}>
      {title}
    </div>
  )
}
  ...
  return (
    <div style={{ display:'flex', alignItems:'center', gap:12, padding:'10px 0', borderBottom:'1px solid #0d0d0d12' }}>
      ...
      <div style={{ flex:1, minWidth:0 }}>
        ...
        <div style={{ fontSize:11, color:'#888', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{entry.youtube_url}</div>
      </div>
